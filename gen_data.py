...
def get_random_dag(n, max_degree=3, states=STATES):
    ...
    for e in edges:
        ...
        g_graph.add_directed_edge(
            GraphNode(u.get_node_name(e[0])), GraphNode(u.get_node_name(e[1]))
        )
...
def generate_data(
    ...
    an_nodes = [
        u.get_node_name(x)
        for x in np.random.choice(nodes, anomalous_nodes, replace=False)
    ]
    if verbose:
    ...
    gumimage.export(
        bn, src_dir + GROUND_TRUTH + ".pdf", nodeColor={n: 0 for n in an_nodes}
    )
